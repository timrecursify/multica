...
Live results below are from the read-only checks recorded on 2026-09-06 (~13:10Z); no DDL/DML was run by this report author. “Preflight” queries are reproducible checks.

### 303 — `qc_attempt_binding_required`

Creates a function and conditional `qc_verdict` trigger; existing function/trigger check: `to_regprocedure('public.require_qc_attempt_binding()')`, `pg_trigger`. Predicate preflight (last seven days) returned `PASS=710`, binding `363`, hence 347 would be rejected. Existing rows are untouched, but future PASS writes are materially affected. Verdict: **UNSAFE** until GSP-2319/GSP-2323 resolves the 49% gap.

### 304 — `relay_run_log`

`CREATE TABLE IF NOT EXISTS` with FK, identity PK, unique task ID, and status check. `to_regclass('public.relay_run_log')` was present with 47,400 rows (created out-of-band by migration 296). Preflight: `SELECT count(*) FROM relay_run_log` → 47,400; no creation work remains. Verdict: **SAFE TO APPLY** (ledger repair/no-op).

### 305 — `relay_stage_pool_invocation_grant`

Updates enabled, unarchived pool agents and inserts missing workspace invocation targets. Preflights returned 36 enabled agents, 0 requiring permission/visibility changes, and 0 missing targets. Verdict: **SAFE TO APPLY** (semantic no-op; recheck immediately before execution).

### 306 — `agent_task_queue_updated_at_invariant`

Adds/backfills `updated_at`, default/NOT NULL, touch trigger, and a CHECK. The first attempted apply rolled back in 0.17s because 13 legacy rows cannot be updated under `agent_task_queue_active_requires_runtime` (10 failed, 3 completed). Before backfill, 29,556 rows violated the new check; projected after backfill: 0. Table is belt-critical and currently has 32,997/35,124 stale timestamps. Verdict: **SAFE AFTER REMEDIATION**: ship the targeted skip for those 13 rows (PR #579), then rerun and verify the post-backfill count.

### 308 — `issue_funnel_activity`

Creates `issue_funnel_transition`, two issue triggers, and a duration view. `to_regclass('public.issue_funnel_transition')` returned absent. This is a new write-path trigger on hot `issue`; no orphan/uniqueness preflight applies. Table lock is creation-time `ACCESS EXCLUSIVE`; duration depends on issue cardinality. Verdict: **UNDETERMINED** pending quiet-window load/trigger review.

### 309 — `issue_funnel_activity_index`

Runs `CREATE INDEX CONCURRENTLY` on `issue_funnel_transition(workspace_id, occurred_at DESC)` and therefore depends on 308. The target table/index were absent. Concurrent build avoids a table write lock but requires its own non-transactional execution and still consumes I/O proportional to table size. Verdict: **UNDETERMINED** until 308 is accepted and a quiet-window plan exists.
