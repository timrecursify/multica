...
## Batch 8 — historical structured-URL recovery

- Root cause status: confirmed and addressed. When the current completion has no full PR URL, the durable authenticated provenance can exist only in an earlier completed task's structured `result.pr_url`; prose such as `PR #N` remains untrusted.
