...
WHERE runtime_id = ANY($1::uuid[])
  AND status = 'queued'
  AND (daemon_id IS NULL OR daemon_id = $2::text)
ORDER BY priority DESC, created_at ASC
`
...
func (q *Queries) ListQueuedClaimCandidatesByRuntimes(ctx context.Context, runtimeIds []pgtype.UUID, daemonId string) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listQueuedClaimCandidatesByRuntimes, runtimeIds, daemonId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
