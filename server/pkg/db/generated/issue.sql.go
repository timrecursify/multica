...
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, dedupe_key FROM issue
WHERE workspace_id = $1
...
const findFuzzyDuplicateIssues = `-- name: FindFuzzyDuplicateIssues :many
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, dedupe_key FROM issue
WHERE workspace_id = $1
...
type FindFuzzyDuplicateIssuesParams struct {
	WorkspaceID   pgtype.UUID `json:"workspace_id"`
...
func (q *Queries) FindFuzzyDuplicateIssues(ctx context.Context, arg FindFuzzyDuplicateIssuesParams) ([]Issue, error) {
	rows, err := q.db.Query(ctx, findFuzzyDuplicateIssues, arg.WorkspaceID, arg.Lower, arg.MinSimilarity)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const findIssueByDedupeKey = `-- name: FindIssueByDedupeKey :one
SELECT id, workspace_id, title, description, status, priority, assignee_type, assignee_id, creator_type, creator_id, parent_issue_id, acceptance_criteria, context_refs, position, due_date, created_at, updated_at, number, project_id, origin_type, origin_id, first_executed_at, start_date, metadata, stage, properties, dedupe_key FROM issue
WHERE workspace_id = $1
  AND dedupe_key = $2
  AND status NOT IN ('Done', 'done', 'Cancelled', 'cancelled', 'Archived')
...
type FindIssueByDedupeKeyParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	DedupeKey   pgtype.Text `json:"dedupe_key"`
}
...
func (q *Queries) FindIssueByDedupeKey(ctx context.Context, arg FindIssueByDedupeKeyParams) (Issue, error) {
	row := q.db.QueryRow(ctx, findIssueByDedupeKey, arg.WorkspaceID, arg.DedupeKey)
	var i Issue
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.assignee_type, i.assignee_id, i.creator_type, i.creator_id, i.parent_issue_id, i.acceptance_criteria, i.context_refs, i.position, i.due_date, i.created_at, i.updated_at, i.number, i.project_id, i.origin_type, i.origin_id, i.first_executed_at, i.start_date, i.metadata, i.stage, i.properties, i.dedupe_key FROM issue i
WHERE i.workspace_id = $1
