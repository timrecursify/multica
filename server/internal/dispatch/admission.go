...
// nearLimit reports whether either resource sits at or above its own
// AlertThreshold fraction. Queue depth and active concurrency are different
// units and must not be compared against a shared (or smaller) cap.
func nearLimit(cfg AdmissionPolicy, load Load) bool {
	if cfg.AlertThreshold <= 0 {
		return false
	}
	queueNear := cfg.MaxQueueDepth > 0 && float64(load.QueueDepth) >= cfg.AlertThreshold*float64(cfg.MaxQueueDepth)
	concurrentNear := cfg.MaxConcurrent > 0 && float64(load.ActiveConcurrent) >= cfg.AlertThreshold*float64(cfg.MaxConcurrent)
	return queueNear || concurrentNear
}
