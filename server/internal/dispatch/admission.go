...
// nearLimit reports whether either configured resource sits at or above its
// own AlertThreshold watermark. Queue depth and active concurrency are
// different units and must not be compared against a shared/minimum cap.
func nearLimit(cfg AdmissionPolicy, load Load) bool {
	...
	activeNear := cfg.MaxConcurrent > 0 && float64(load.ActiveConcurrent) >= cfg.AlertThreshold*float64(cfg.MaxConcurrent)
