...
func TestRestoreCanonicalIssueStatusCheckMigrationPreservesData(t *testing.T) {
	...
	if _, err := conn.Exec(ctx, `INSERT INTO issue (id, status) VALUES ('00000000-0000-0000-0000-000000000011', 'in_progress')`); err != nil {
		t.Fatalf("insert legacy status through compatibility trigger: %v", err)
	}
	assertIssueStatuses(t, ctx, conn.Conn(), map[string]string{
		"00000000-0000-0000-0000-000000000011": "In Progress",
	})
	if _, err := conn.Exec(ctx, `INSERT INTO issue (id, status) VALUES ('00000000-0000-0000-0000-000000000012', 'unknown')`); !isCheckViolation(err) {
