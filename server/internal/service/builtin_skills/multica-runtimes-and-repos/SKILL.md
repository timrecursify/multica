...
- `daemon status` and `daemon disk-usage` report on the runtime hosting this task: `status` probes the daemon-injected health port, and `disk-usage` scans the daemon-injected workspaces root. Both refuse `--profile`, and `disk-usage` also refuses `--all-profiles` and `--workspaces-root` by default (the error names the triggering env/marker). For an auditable GC reproduction, pass `--allow-in-task --workspaces-root <explicit-root>`; this does not enable profile or all-profile enumeration, and STATUS stays blank because filling it would spend the Owner's credential. The systemd service runs from `cwd=/`, so it does not need this escape.
