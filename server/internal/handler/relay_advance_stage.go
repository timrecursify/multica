...
func (h *Handler) RelayAdvanceStage(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		writeErrorJSON(w, http.StatusBadRequest, relayError{
			Error: "invalid_to_stage", Message: "to_stage is not a configured relay stage",
			FromStage: issue.Status, ToStage: req.ToStage,
		})
		return
	}
	if err != nil {
	...
	default:
		slog.Error("relay advance-stage: enqueue successor task failed",
			"issue_id", uuidToString(issue.ID), "agent_id", uuidToString(owner.AgentID), "error", taskErr)
		writeError(w, http.StatusInternalServerError, "failed to enqueue successor task")
		return
	}
