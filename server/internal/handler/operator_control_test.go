...
func TestListGetSetRelayStageOwner(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("no test database")
	}

	// List.
	req := newRequest("GET", "/api/operator/workspaces/{workspaceId}/relay/", nil)
	req = withURLParam(req, "workspaceId", testWorkspaceID)
	rr := httptest.NewRecorder()
	testHandler.ListWorkspaceRelayStages(rr, req)
	if rr.Code != http.StatusOK {
	...
	if !ok || len(stages) == 0 {
		t.Fatalf("expected relay stages, got %v", body)
	}

	// Set owner to a workspace agent.
	ownerID := createHandlerTestAgent(t, "operator-relay-owner", nil)
	t.Cleanup(func() { testPool.Exec(context.Background(), `DELETE FROM agent WHERE id = $1`, ownerID) })
	req = newRequest("PUT", "/api/operator/workspaces/{workspaceId}/relay/{stageName}/owner", map[string]any{
		"agent_id": ownerID,
	})
	req = withURLParams(req, "workspaceId", testWorkspaceID, "stageName", "Spec")
	rr = httptest.NewRecorder()
	testHandler.SetWorkspaceRelayStageOwner(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("set-owner status = %d, want 200 (body %s)", rr.Code, rr.Body.String())
	}
	body = decodeOperatorResponse(t, rr)
	t.Cleanup(func() {
		req := newRequest("PUT", "/api/operator/workspaces/{workspaceId}/relay/{stageName}/owner", map[string]any{"agent_id": ""})
		req = withURLParam(req, "workspaceId", testWorkspaceID)
		req = withURLParam(req, "stageName", "Spec")
		rr := httptest.NewRecorder()
		testHandler.SetWorkspaceRelayStageOwner(rr, req)
	})
	if body["agent_id"] != ownerID {
		t.Fatalf("owner agent_id = %v, want %s", body["agent_id"], ownerID)
	}

	// What about an unknown stage -> 404.
	req = newRequest("PUT", "/api/operator/workspaces/{workspaceId}/relay/{stageName}/owner", map[string]any{"agent_id": ""})
	req = withURLParams(req, "workspaceId", testWorkspaceID, "stageName", "NoSuchStage")
	rr = httptest.NewRecorder()
	testHandler.SetWorkspaceRelayStageOwner(rr, req)
	if rr.Code != http.StatusNotFound {
		t.Fatalf("unknown-stage status = %d, want 404", rr.Code)
	}
	...
	testHandler.SetWorkspaceRelayStageOwner(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("clear-owner status = %d, want 200 (body %s)", rr.Code, rr.Body.String())
	}
	body = decodeOperatorResponse(t, rr)
	if body["agent_id"] != nil {
		t.Fatalf("expected cleared owner, got %v", body["agent_id"])
	}
}
