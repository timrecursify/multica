...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	r.Route("/api/operator", func(r chi.Router) {
		...
		r.Route("/workspaces/{workspaceId}/relay", func(r chi.Router) {
			r.Get("/", h.ListWorkspaceRelayStages)
			r.Get("/{stageName}", h.GetWorkspaceRelayStage)
			r.Put("/{stageName}/owner", h.SetWorkspaceRelayStageOwner)
		})
	})
	...
	r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// Live dashboard metrics derived from status transitions and
			// configured relay stages rather than retired task-queue rows.
			r.Get("/api/dashboard/throughput", h.GetWorkspaceThroughput)
			r.Get("/api/dashboard/stage-counts", h.ListWorkspaceStageCounts)
