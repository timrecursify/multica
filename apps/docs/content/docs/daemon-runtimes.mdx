...
1. Run `multica daemon status` to confirm the selected daemon is running. The result is scoped to the user profile or current managed task, not externally supervised daemons; check the supervisor-specific health surface for those deployments.
