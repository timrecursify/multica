...
`daemon status` reports only the selected user-profile daemon (`scope: user_profile`), or the daemon hosting the current managed task (`scope: managed_task`). It is not a host-wide or supervisor-wide health check; supervised deployments should also use their supervisor-specific health surface.
