...
Edit `.env` — at minimum, change `JWT_SECRET` (generate one with
`openssl rand -hex 32`) and the Postgres password. Then start the stack:
...
1. Requires `.env` — a missing or unreadable file fails with an actionable, secret-free message. It never creates or overwrites `.env`
2. Pulls the PostgreSQL, Multica backend, and Multica frontend images
3. Creates persistent volumes and starts the three containers
4. Waits until the backend starts answering health checks

Running `make selfhost` again reuses the existing `.env` and volumes. Compose runs through `scripts/selfhost-compose.sh` with `--env-file .env` from a sanitized environment, so inherited shell values and `make selfhost PORT=…` overrides cannot change the rendered stack — edit `.env` instead.
...
Two ways to upgrade a Docker Compose install. On an existing install they produce the **same result** — the `selfhost` target in the `Makefile` runs the same `docker compose pull` + `up -d` (through `scripts/selfhost-compose.sh`, which requires `.env` and reads configuration only from it) and waits on `/health` before printing a status summary. Pick whichever you prefer:
...
`make selfhost` **never creates or overwrites `.env`** — it requires the file to exist and fails with an actionable message when it is missing. Re-running it on an existing install leaves your `JWT_SECRET`, Postgres password, email settings, and `FRONTEND_ORIGIN` exactly as they were.
