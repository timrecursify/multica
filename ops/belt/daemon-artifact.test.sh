...
EOF
chmod +x "$tmp/pm2"
reset(){ cp "$tmp/old-server" "$tmp/runtime/server"; cp "$tmp/old-server" "$tmp/proc/77/exe"; printf 'server\0daemon\0start\0--max-concurrent-tasks=32\0' >"$tmp/proc/77/cmdline"; : >"$tmp/reloads"; manifest; }
