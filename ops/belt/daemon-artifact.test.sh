...
case "$1" in
  ...
  reload)
    ...
    if [[ "${CORRUPT_BACKUP:-0}" == 1 && "$count" -eq 1 ]]; then backup="$(find "$(dirname "$TARGET")" -maxdepth 1 -name "$(basename "$TARGET").bak-*" -print -quit)"; printf corrupt >"$backup"; fi
    if [[ ( "${RELOAD_FAIL:-0}" == 1 && "$count" -eq 1 ) || ( "${ROLLBACK_RELOAD_FAIL:-0}" == 1 && "$count" -gt 1 ) ]]; then exit 1; fi
    cp "$TARGET" "$PROC_ROOT/77/exe"
    if [[ "${WRONG_CAP:-0}" == 1 && "$count" -eq 1 ]]; then printf 'server daemon start --max-concurrent-tasks=2\0' >"$PROC_ROOT/77/cmdline"; else printf 'server daemon start --max-concurrent-tasks=32\0' >"$PROC_ROOT/77/cmdline"; fi;;
esac
EOF
chmod +x "$tmp/pm2"
reset(){ cp "$tmp/old-server" "$tmp/runtime/server"; cp "$tmp/old-server" "$tmp/proc/77/exe"; printf 'server daemon start --max-concurrent-tasks=32\0' >"$tmp/proc/77/cmdline"; : >"$tmp/reloads"; manifest; }
