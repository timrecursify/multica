...
async function recordOneOutcome(client, row, logger, { githubCommand } = {}) {
  ...
  if (parsed.outcome === "ADVANCED" && row.stage === "In Progress") {
    ...
    if (!evidence?.has_review_evidence) {
      const issue = (await client.query(`SELECT id, workspace_id, status FROM issue WHERE id = $1::uuid`, [row.issue_id])).rows[0];
      if (issue) {
        const { linkObservedPullRequest } = require("./reconciler.cjs");
        await linkObservedPullRequest(client, issue, { githubCommand });
      }
      const refreshed = (await client.query(`SELECT EXISTS (SELECT 1 FROM issue_pull_request ipr JOIN github_pull_request p ON p.id = ipr.pull_request_id WHERE ipr.issue_id = $1::uuid AND NULLIF(p.head_sha, '') IS NOT NULL) AS has_review_evidence`, [row.issue_id])).rows[0];
