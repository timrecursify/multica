...
disk_out="$(PATH="$fixture/bin:$PATH" FAKE_DOCKER_ARGS="$fixture/docker.args" FAKE_DESCRIPTOR="$descriptor"$'\n'"$descriptor" WORKSPACE_GC_BATCH_LIMIT=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_TEST_MODE=1 BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" "$dir/workspace-gc.sh" 2>"$fixture/disk.err")"
...
busy_out="$(BELT_TEST_MODE=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh")"
! grep -q "$task" <<<"$busy_out"
kill "$busy_pid"; wait "$busy_pid" 2>/dev/null || :
out="$(BELT_TEST_MODE=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh")"
grep -q "$task" <<<"$out"
...
blocked2=72345678-1234-1234-1234-123456789abc
later_clean=82345678-1234-1234-1234-123456789abc
...
make_candidate "$blocked2" dirty
make_candidate "$later_clean" clean
printf '%s\n' \
...
printf '%s\n' \
  "$blocked"$'\tcompleted\t2026-01-01T00:00:00Z\t'"$issue"$'\t'"$gcroot/$workspace/${blocked:0:8}/workdir" \
  "$blocked2"$'\tcompleted\t2026-01-01T00:00:01Z\t'"$issue"$'\t'"$gcroot/$workspace/${blocked2:0:8}/workdir" \
  "$later_clean"$'\tcompleted\t2026-01-01T00:00:02Z\t'"$issue"$'\t'"$gcroot/$workspace/${later_clean:0:8}/workdir" >"$fixture/starvation-descriptors"
mkdir -p "$gcstate"
printf '{"tasks":{"%s":{"retry_after":9999999999,"failures":1,"first_blocked_at":1},"%s":{"retry_after":9999999999,"failures":1,"first_blocked_at":1}}}\n' "$blocked" "$blocked2" >"$gcstate/.gc-blocked.json"
BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_BATCH_LIMIT=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/starvation-descriptors" "$dir/workspace-gc.sh" >"$fixture/starvation.out"
grep -q "$later_clean" "$fixture/starvation.out"
[[ ! -e "$gcroot/.gc-blocked.json" ]]
real_git="$(command -v git)"
printf '%s\n' '#!/usr/bin/env bash' 'case " $* " in *62345678*" status "*) exit 42;; esac' 'exec "$REAL_GIT" "$@"' >"$fixture/bin/git"
chmod +x "$fixture/bin/git"
PATH="$fixture/bin:$PATH" REAL_GIT="$real_git" BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh" >"$fixture/status-failure.out"
! grep -q "$statusfail" "$fixture/status-failure.out"
rm "$fixture/bin/git"
PATH="$fixture/bin:$PATH" BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" WORKSPACE_GC_TIME_LIMIT_SECONDS=30 "$dir/workspace-gc.sh" >"$fixture/first.out"
grep -q "$clean" "$fixture/first.out"
! grep -q "$blocked\|$local\|$stash" "$fixture/first.out"
jq -e --arg id "$blocked" '.tasks[$id].retry_after > 0' "$gcstate/.gc-blocked.json" >/dev/null
PATH="$fixture/bin:$PATH" BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh" >"$fixture/second.out"
grep -q "$clean" "$fixture/second.out"
! grep -q "$blocked" "$fixture/second.out"
jq --arg id "$blocked" '.tasks[$id].retry_after = 0' "$gcstate/.gc-blocked.json" >"$fixture/state.tmp" && mv "$fixture/state.tmp" "$gcstate/.gc-blocked.json"
PATH="$fixture/bin:$PATH" BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh" >"$fixture/retry.out"
! grep -q "$blocked\|$local\|$stash" "$fixture/retry.out"
printf '{broken' >"$gcstate/.gc-blocked.json"
PATH="$fixture/bin:$PATH" BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh" >"$fixture/corrupt.out"
grep -q "$clean" "$fixture/corrupt.out"

du_fail="$fixture/bin/du"
printf '%s\n' '#!/usr/bin/env bash' 'exit 99' >"$du_fail"
chmod +x "$du_fail"
PATH="$fixture/bin:$PATH" BELT_TEST_MODE=1 WORKSPACE_GC_SKIP_BUSY_SCAN=1 WORKSPACE_GC_STATE_DIR="$gcstate" BELT_WORKSPACES_ROOT_OVERRIDE="$gcroot" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh" >"$fixture/no-du.out"
