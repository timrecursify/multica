...
mkdir -p "$fixture/remote.git" "$fixture/$workspace/${task:0:8}" "$fixture/proc"
...
out="$(BELT_TEST_MODE=1 WORKSPACE_GC_PROC_ROOT="$fixture/proc" BELT_WORKSPACES_ROOT_OVERRIDE="$fixture" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh")"
grep -q "$task" <<<"$out"
BELT_TEST_MODE=1 WORKSPACE_GC_PROC_ROOT="$fixture/proc" BELT_WORKSPACES_ROOT_OVERRIDE="$fixture" WORKSPACE_GC_DESCRIPTOR_FILE="$fixture/descriptors" "$dir/workspace-gc.sh" --apply >/dev/null
