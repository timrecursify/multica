...
source_rels+=(ops/belt/parked-runtime-verification.cjs)
targets+=("$runtime_root/gsp-multica/parked-runtime-verification.cjs")
new_runtime_target=1
...
for i in "${!targets[@]}"; do selected "$i" || continue; new_targets[$i]=0; [[ "${targets[$i]}" =~ /(guardrails|parked-diagnosis|parked-entry-audit|parked-runtime-verification|relay-dead-rows)\.cjs$ || "${targets[$i]}" =~ /(cicd-deploy-evidence|relay-completion-admission|qc-lane|reconciler|stage-outcome|transition-policy|stage-routing|qc-strict-evidence|stage-routing.|qc-verdict-policy)\.cjs$ || "${targets[$i]}" =~ /stage-routing\.json$ ]] && new_targets[$i]=1; if [[ "$mode" != rollback && ! -f "${staged_sources[$i]}" && ${new_targets[$i]} -eq 0 ]]; then echo "Missing selected commit blob: ${source_rels[$i]}" >&2; invalid=1; fi; [[ "$mode" == rollback || -f "${targets[$i]}" || ${new_targets[$i]} -eq 1 ]] || { echo "Missing runtime file: ${targets[$i]}" >&2; invalid=1; }; [[ "$mode" != rollback || -f "${targets[$i]}.bak-${rollback_timestamp}" || -f "${targets[$i]}.bak-${rollback_timestamp}.absent" ]] || { echo "Missing rollback backup: ${targets[$i]}.bak-${rollback_timestamp}" >&2; invalid=1; }; done
(( ! invalid )) || exit 1
