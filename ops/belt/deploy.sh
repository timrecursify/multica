...
declare -a source_rels=(ops/belt/multica-bridge.cjs ops/belt/guardrails.cjs ops/belt/parked-diagnosis.cjs ops/belt/parked-entry-audit.cjs ops/belt/parity/multica-relay-advance-daemon.cjs ops/belt/parity/relay-dead-rows.cjs ops/belt/stage-routing.cjs ops/belt/stage-routing.json ops/belt/qc-strict-evidence.cjs ops/belt/qc-lane.cjs ops/belt/reconciler.cjs ops/belt/stage-outcome.cjs ops/belt/transition-policy.cjs ops/belt/qc-verdict-policy.cjs ops/belt/multica-cicd-worker.cjs ops/belt/cicd-deploy-evidence.cjs ops/belt/multica-archiver.cjs ops/belt/belt-config-guard.sh ops/belt/multica-daemon-wrapper.sh ops/belt/ecosystem.gsp-belt.config.js ops/belt/multica-bundle.py ops/belt/RUNBOOK_SPEC_WORKER.md ops/belt/RUNBOOK_BUILD_WORKER.md ops/belt/RUNBOOK_QC_WORKER.md ops/belt/WORKER_COMMON.md ops/belt/relay-completion-admission.cjs)
declare -a targets=("$runtime_root/gsp-multica/multica-bridge.cjs" "$runtime_root/gsp-multica/guardrails.cjs" "$runtime_root/gsp-multica/parked-diagnosis.cjs" "$runtime_root/gsp-multica/parked-entry-audit.cjs" "$runtime_root/gsp-multica/parity/multica-relay-advance-daemon.cjs" "$runtime_root/gsp-multica/parity/relay-dead-rows.cjs" "$runtime_root/gsp-multica/stage-routing.cjs" "$runtime_root/gsp-multica/stage-routing.json" "$runtime_root/gsp-multica/qc-strict-evidence.cjs" "$runtime_root/gsp-multica/qc-lane.cjs" "$runtime_root/gsp-multica/reconciler.cjs" "$runtime_root/gsp-multica/stage-outcome.cjs" "$runtime_root/gsp-multica/transition-policy.cjs" "$runtime_root/gsp-multica/qc-verdict-policy.cjs" "$runtime_root/multica-cicd-worker.cjs" "$runtime_root/cicd-deploy-evidence.cjs" "$runtime_root/multica-archiver.cjs" "$runtime_root/tools/belt-config-guard.sh" "$runtime_root/gsp-multica/fleet/multica-daemon-wrapper.sh" "$runtime_root/gsp-multica/fleet/ecosystem.gsp-belt.config.js" "$runtime_root/tools/multica-bundle.py" "$runtime_root/multica-doctrine/RUNBOOK_SPEC_WORKER.md" "$runtime_root/multica-doctrine/RUNBOOK_BUILD_WORKER.md" "$runtime_root/multica-doctrine/RUNBOOK_QC_WORKER.md" "$runtime_root/multica-doctrine/WORKER_COMMON.md" "$runtime_root/gsp-multica/relay-completion-admission.cjs")
...
for i in "${!targets[@]}"; do selected "$i" || continue; new_targets[$i]=0; [[ "${targets[$i]}" =~ /(guardrails|parked-diagnosis|parked-entry-audit|relay-dead-rows)\.cjs$ || "${targets[$i]}" =~ /(cicd-deploy-evidence|relay-completion-admission)\.cjs$ ]] && new_targets[$i]=1; if [[ "$mode" != rollback && ! -f "${staged_sources[$i]}" ]]; then echo "Missing selected commit blob: ${source_rels[$i]}" >&2; invalid=1; fi; [[ "$mode" == rollback || -f "${targets[$i]}" || ${new_targets[$i]} -eq 1 ]] || { echo "Missing runtime file: ${targets[$i]}" >&2; invalid=1; }; [[ "$mode" != rollback || -f "${targets[$i]}.bak-${rollback_timestamp}" || -f "${targets[$i]}.bak-${rollback_timestamp}.absent" ]] || { echo "Missing rollback backup: ${targets[$i]}.bak-${rollback_timestamp}" >&2; invalid=1; }; done
(( ! invalid )) || exit 1
