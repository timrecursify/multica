...
md5() { [[ -f "$1" ]] && md5sum -- "$1" | awk '{print $1}' || true; }
last_deployed_sha() {
...
for i in "${!targets[@]}"; do
  ...
  [[ -z "$last_sha" ]] || last_md5="$(git -C "$repo_root" cat-file -e "$last_sha:${source_rels[$i]}" 2>/dev/null && git -C "$repo_root" show "$last_sha:${source_rels[$i]}" | md5sum | awk '{print $1}' || true)"
