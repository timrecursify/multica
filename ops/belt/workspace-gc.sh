...
min_age=2; apply=0; max_items="${WORKSPACE_GC_MAX_ITEMS:-100}"; max_bytes="${WORKSPACE_GC_MAX_BYTES:-10737418240}"
while (($#)); do case "$1" in --apply) apply=1;; --min-age-hours) min_age="$2"; shift;; --max-items) max_items="$2"; shift;; --max-bytes) max_bytes="$2"; shift;; *) exit 64;; esac; shift; done
root="$(workspace_root_resolve)"; cutoff=$(( $(date +%s) - min_age * 3600 )); total=0; paths=(); bytes=()
keep="${KEEP_WORKDIR:-}"; [[ -n "$keep" && "$keep" != 0 && "${keep,,}" != false && "${keep,,}" != no ]] || keep=""
while IFS= read -r -d '' task; do
  ...
  (( busy == 0 )) || continue; [[ -z "$keep" ]] || continue; size="$(du -sb -- "$task" | awk '{print $1}')"
  (( ${#paths[@]} < max_items && total + size <= max_bytes )) || continue
  paths+=("$task"); bytes+=("$size"); total=$((total + size)); printf '%s %s\n' "$task" "$size"
