...
function createWatchdog({ file, now = () => Date.now() } = {}) {
  ...
  const observe = (issueId, { stage = 'CI/CD & Deploy', sha = '', outcome, error, increment = true } = {}) => {
    ...
    if (increment) { row.last_attempt_at = row.last_seen_at; row.attempts = Math.min(RETRY_LIMIT, row.attempts + 1); }
  ...
  const observePresence = (issueId, opts = {}) => observe(issueId, { ...opts, increment: false });
  const observeFailure = (issueId, opts = {}) => observe(issueId, { ...opts, increment: true });
  return { observe, observePresence, observeFailure, retryAllowed, backoffMs, stalled, markAlerted, clear, snapshot: () => ({ ...state }) };
}
