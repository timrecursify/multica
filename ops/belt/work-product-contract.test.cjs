...
test("build outcome atomically inserts ownership and rework updates its exact identity against PostgreSQL", {
  ...
  try {
    ...
    await client.query("CREATE TABLE relay_run_log (issue_id uuid, task_id uuid, from_stage text, to_stage text, created_at timestamptz)");
    ...
    const insertTask = async (id, sha) => {
      ...
      await client.query("INSERT INTO relay_run_log VALUES ($1,$2,'In Progress','In Review',NOW())", [issueId, id]);
    };
