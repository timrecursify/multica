...
root_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"; repo_root="$(cd -- "$root_dir/../.." && pwd)"; runtime_root="${BELT_DEPLOY_RUNTIME_ROOT:-/opt/gsp/multica-workers}"
...
# Source paths are derived from the manifest so the two can never diverge.
. "$root_dir/belt-manifest.sh"
declare -a rels=(); for i in "${!sources[@]}"; do rels[i]="ops/belt/${sources[$i]#"$root_dir/"}"; done
