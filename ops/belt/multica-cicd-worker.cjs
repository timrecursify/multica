...
async function watchdogFailure(issue, error, sha = '') {
  const row = watchdog.observeFailure(issue.id, { sha, outcome: 'retrying', error });
...
async function closureWatchdog(issue, result, sha) {
  ...
  if (result.retryEligible && ['pending', 'discovery_unavailable'].includes(result.outcome)) {
    watchdog.observeFailure(issue.id, { sha, outcome: result.outcome, error: result.blocker?.type });
    return false;
  }
  const row = watchdog.observeFailure(issue.id, { sha, outcome: 'closure_pending' });
