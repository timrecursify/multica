...
async function retryEscalation(issue, toStage, reason, evidence = {}, sourceTaskId = null) {
  ...
  await relay(issue.id, toStage, null, reason, null, retryEvidence, sourceTaskId);
}

async function watchdogFailure(issue, error, sha = '', sourceTaskId = null) {
  ...
  if (watchdog.stalled(row)) {
    ...
    await relay(issue.id, 'Spec', null, detail, null, evidence, sourceTaskId);
...
async function sweep() {
  ...
  const { rows } = await pool.query(
    `SELECT i.id, i.number, i.title, i.workspace_id, i.metadata,
       (SELECT r.task_id FROM relay_run_log r WHERE r.issue_id=i.id AND r.to_stage='CI/CD & Deploy' ORDER BY r.created_at DESC LIMIT 1) AS source_task_id
  ...
  for (let issueIndex = 0; issueIndex < rows.length; issueIndex++) {
    ...
    try {
      ...
      try {
        failure = await watchdogFailure(issue, e.message, '', issue.source_task_id);
