...
cat >"$tmp/bin/pm2" <<'EOF'
#!/usr/bin/env bash
case "$1" in jlist) printf '[{"name":"gsp-multica-bridge","pid":1,"pm2_env":{"status":"online","pm_cwd":"%s","pm_exec_path":"%s/ops/belt/multica-bridge.cjs"}},{"name":"multica-relay-advance","pm2_env":{"status":"online","pm_cwd":"%s","pm_exec_path":"%s/ops/gsp-belt/relay/multica-relay-advance-wrapper.sh"}},{"name":"gsp-multica-worker","pm2_env":{"status":"online","pm_cwd":"%s","pm_exec_path":"%s/ops/belt/multica-daemon-wrapper.sh"}},{"name":"multica-cicd-worker","pm2_env":{"status":"online","pm_cwd":"%s","pm_exec_path":"%s/ops/belt/multica-cicd-worker.cjs"}},{"name":"multica-archiver","pm2_env":{"status":"online","pm_cwd":"%s","pm_exec_path":"%s/ops/belt/multica-archiver.cjs"}}]' "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE" "$EXPECT_RELEASE";; startOrReload) printf '%s %s\n' "${MULTICA_INCLUDE_WORKER:-0}" "${MULTICA_SKIP_CICD_WORKER:-0}" >> "$PM2_LOG";; save) echo save >> "$PM2_LOG";; *) exit 0;; esac
EOF
