...
if [[ -z "$old_sha" || "$old_sha" == "$main_sha" ]]; then
  # Some hosted checkouts expose only the tip bridge revision; its parent is
  # still a deterministic, older runtime snapshot for the replay.
