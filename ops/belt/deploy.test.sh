...
for rel in "${files[@]}"; do
  [[ "$rel" == gsp-multica/guardrails.cjs || "$rel" == gsp-multica/parked-diagnosis.cjs ]] && continue
...
for rel in "${files[@]}"; do
  target="$tmp_dir/$rel"
  if [[ "$rel" == gsp-multica/guardrails.cjs || "$rel" == gsp-multica/parked-diagnosis.cjs ]]; then
