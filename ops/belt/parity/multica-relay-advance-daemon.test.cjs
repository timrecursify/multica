...
test('a 409 relay refusal preserves the typed outcome instead of rewriting it as human', async () => {
  const calls = [];
  const logs = [];
  const client = { release() {}, query: async (sql, values) => {
    calls.push({ sql, values });
  ...
  assert.equal(parked, undefined, 'relay refusal must not rewrite issue_stage_outcome');
