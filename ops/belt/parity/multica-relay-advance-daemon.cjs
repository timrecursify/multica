...
async function recordRefusedAdvance(client, row) {
  await markRelayLogFailedById(client, row.log_id);
  // Relay refusal is transport/routing state, not a task outcome.  In
  // particular it must never overwrite a typed ADVANCED/NO_OP with
  // FAILED/human; explicit human blockers are produced only by the worker.
}
...
async function readvanceRecordedOutcomes({ dbPool = pool, postRelay = postToRelay,
  ...
  try {
    ...
    for (const row of result.rows) {
      ...
      // Keep the successful task outcome authoritative even after refusal or
      // retry exhaustion. Diagnostics above remain in relay_run_log.
