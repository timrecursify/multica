...
async function convertCompletedQcEvidence(client, { postRelay, logger = console,
  ...
  for (const task of candidates.rows) {
    ...
    // Load only when conversion is enabled: daemon unit tests do not need the
    // bridge's mandatory runtime configuration to exercise dead-row behavior.
    const { qcTaskEvidence } = require('../multica-bridge.cjs');
...
async function closeDeadRelayRows(client, { terminalStages, requestRetryEscalation,
  postRelay, postVerdict = postRelay, logger = console, logPrefix = '[relay-advance-daemon]', env = process.env }) {
