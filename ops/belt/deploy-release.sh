...
manifest=(ops/belt/multica-bridge.cjs ops/belt/build-admission.cjs ops/belt/guardrails.cjs ops/belt/human-review-routing.cjs ops/belt/astra-adjudication.cjs ops/belt/parked-diagnosis.cjs ops/belt/parked-entry-audit.cjs ops/belt/parity/multica-relay-advance-daemon.cjs ops/belt/parity/relay-dead-rows.cjs ops/belt/multica-cicd-worker.cjs ops/belt/cicd-watchdog.cjs ops/belt/cicd-deploy-evidence.cjs ops/belt/multica-archiver.cjs ops/belt/reconciler.cjs ops/belt/stage-outcome.cjs ops/belt/transition-policy.cjs ops/belt/stage-routing.cjs ops/belt/qc-strict-evidence.cjs ops/belt/qc-verdict-policy.cjs ops/belt/belt-config-guard.sh ops/belt/multica-daemon-wrapper.sh ops/belt/ecosystem.gsp-belt.config.js ops/belt/multica-bundle.py ops/belt/RUNBOOK_SPEC_WORKER.md ops/belt/RUNBOOK_ASTRA_ADJUDICATOR.md ops/belt/RUNBOOK_BUILD_WORKER.md ops/belt/RUNBOOK_QC_WORKER.md ops/belt/WORKER_COMMON.md ops/belt/relay-completion-admission.cjs ops/gsp-belt/relay/multica-relay-advance-launcher.cjs ops/gsp-belt/relay/multica-relay-advance-wrapper.sh ops/gsp-belt/relay/multica-relay-advance-daemon.cjs)
health() {
  local snapshot
  ...
  RELEASE="$release" INCLUDE_WORKER="$include_worker" SKIP_CICD="$skip_cicd_worker" PM2_SNAPSHOT="$snapshot" node -e '
    ...
    const required=["gsp-multica-bridge","gsp-multica-worker","multica-relay-advance","multica-cicd-worker","multica-archiver"];
    const expected={"gsp-multica-worker":`${release}/ops/belt/multica-daemon-wrapper.sh`,"multica-relay-advance":`${release}/ops/gsp-belt/relay/multica-relay-advance-wrapper.sh`,"multica-cicd-worker":`${release}/ops/belt/multica-cicd-worker.cjs`,"multica-archiver":`${release}/ops/belt/multica-archiver.cjs`}, byName=new Map(apps.map(a=>[a.name,a])); let ok=true;
    for(const name of required){const app=byName.get(name),e=app?.pm2_env||{}, pathOk=!expected[name]||e.pm_exec_path===expected[name];
      if(!app||e.status!=="online"||e.pm_cwd!==release||!pathOk){console.error(`health: app=${name} status=${e.status||"missing"} pid=${app?.pid??"unknown"} exit_code=${e.exit_code??"unknown"} exit_signal=${e.exit_signal||"unknown"} restarts=${e.unstable_restarts??"unknown"} error_log=${e.pm_err_log_path||"unknown"} expected_script=${expected[name]||"release cwd"}`);ok=false;}}
    process.exit(ok?0:1);'
}
...
if ! MULTICA_INCLUDE_WORKER="$include_worker" MULTICA_SKIP_CICD_WORKER="$skip_cicd_worker" "$pm2_bin" startOrReload "$ecosystem" --update-env || ! health || ! "$pm2_bin" save; then
