...
async function sweep() {
  const candidates = await pool.query(
    `SELECT i.id, i.number, pr.head_sha
       FROM issue i JOIN issue_pull_request link ON link.issue_id=i.id
       JOIN github_pull_request pr ON pr.id=link.pull_request_id
      WHERE i.status = ANY(ARRAY['Spec','Queue','In Progress']) AND NOT link.reference_only
        AND pr.state='merged' AND pr.merged_at IS NOT NULL AND pr.head_sha ~ '^[0-9a-fA-F]{40}$'
      ...
      ORDER BY i.id, pr.id LIMIT $1`, [LIMIT]);
