...
async function latestRunningSolLowQcTask(client, issueId, workspaceId, checker, explicitTaskId = null) {
  ...
  return result.rows.length === 1 && result.rows[0].status === 'running' ? result.rows[0] : null;
}
...
async function relayVerdict(req, res, payload) {
  ...
  const payloadError = validateRelayVerdict(payload);
  if (payloadError) {
    relayVerdictError(res, 400, payloadError);
    return;
  }
  ...
  try {
    ...
    const verdict = externalQc ? validateQcVerdict(
      ...
      : liveQcTask ? validateLiveVerdict({ task: { ...liveQcTask, agent: { model: liveQcTask.model,
          thinking_level: liveQcTask.thinking_level, runtime_config: liveQcTask.runtime_config } }, evidence: payload })
      : validateQcVerdict({ actor: { type: 'worker', authenticated_task_id: qcTask.id }, task: {
