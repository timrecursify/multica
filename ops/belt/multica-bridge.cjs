...
async function start() {
  ...
  const client = new Client({ connectionString: MULTICA_DB });
  await client.connect();
  try { await client.query("BEGIN"); await repairPpp23189Qualifying(client); await client.query("COMMIT"); }
  catch (err) { await client.query("ROLLBACK").catch(() => {}); await client.end(); throw err; }
  await client.end();
