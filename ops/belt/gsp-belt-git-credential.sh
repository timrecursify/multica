...
# write, metadata read, workflow write, Actions read, and the two read-only
# commit-check permissions the QC gate needs, nothing else.
#
...
token=$(curl -sS -X POST \
  ...
  -d "{\"repositories\":[\"$repo\"],\"permissions\":{\"contents\":\"write\",\"pull_requests\":\"write\",\"workflows\":\"write\",\"metadata\":\"read\",\"actions\":\"read\",\"checks\":\"read\",\"statuses\":\"read\"}}" \
