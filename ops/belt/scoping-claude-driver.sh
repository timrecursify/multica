...
WITH candidate AS (
  SELECT t.id, t.issue_id, t.agent_id, i.title, COALESCE(i.description, '') AS description
...
while :; do
  ...
  prompt="Produce a concise scoping/spec note for ticket ${issue_id} titled '${title}'. Return text only; do not use tools."
