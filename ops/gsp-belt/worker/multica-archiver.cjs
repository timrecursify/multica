...
const { DATABASE_URL, ARCHIVER_AGENT_SECRET } = process.env;
...
function archiveReceipt(issueID) {
  const payload = `archiver:${issueID}:Done->Archived`;
  return `${payload}:${crypto.createHmac('sha256', ARCHIVER_AGENT_SECRET).update(payload).digest('hex')}`;
}
...
async function archiveDoneIssues() {
  ...
  for (const issue of rows) {
    try { if (await advanceToArchived(issue.id)) archived += 1; }
    catch (error) { console.error(`[multica-archiver] issue=${issue.id} ${error.message}`); }
  }
